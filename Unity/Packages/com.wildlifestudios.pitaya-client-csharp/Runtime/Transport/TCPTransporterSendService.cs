...
{
    internal class TCPTransporterSendService
    {
        ...
        internal TCPTransporterSendService(int timeToSleep, ITransporter transporter, TransporterNotifyService notifier)
        {
        ...
        internal void Start(Stream sendStream)
        {
            ...
            while (_isRunning)
            {
                try
                {
                    if (!_sendingMessage)
                    {
                        ...
                        {
                            PitayaClientLib.Logger.LogDebug("tcp__write_async_cb - Writing to TCP socket");
                            foreach (byte[] buf in buffers)
                            {
                                _sendStream.BeginWrite(buf, 0, buf.Length, new AsyncCallback(SendCallback), null);
        ...
        void SendCallback(IAsyncResult asyncSend)
        {
            if (_isRunning)
            {
                ...
                try
                {
                    _sendStream.EndWrite(asyncSend);
                }
                catch (Exception e)
                {
                    error = true;
                    PitayaClientLib.Logger.LogError("tcp__write_done_cb - uv_write callback error: " + e.Message);
                }
                finally
                {
                    lock (_sendingMessageLock)
                ...
                lock (_writeQueueLock)
                {
                    while (_writingQueue.Count > 0)
                    {
                        ...
                        {
                            PitayaClientLib.Logger.LogDebug(string.Format("tcp__write_done_cb - move wi from writing to resp pending queue,"
